    ...
    if (isset($_POST["name"]) && isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["prenom"])  && isset($_POST["USERNAME"]) && isset($_POST["password1"])) {
        ...
        if (!empty($_POST["name"]) && !empty($_POST["email"]) && !empty($_POST["password"]) && !empty($_POST["prenom"])  && !empty($_POST["USERNAME"]) && !empty($_POST["password1"])) {
            ...
            if ($num == 0 ) {
                if($pass===$pass1)
                {
                $content = $connect->prepare("insert into loginrec (name,email,password,username,prenom) values(?,?,?,?,?)");
                $content->execute(array($name,$email,$pass,$username,$prenom));}
                else
                { echo "<div style='border-radius:20px;color:black;margin-top:20px;margin-right:100px;background-color:red;padding:20px;font-weight:bolder;font-size:larger;text-transform:uppercase'> verify your password please,it seems they are not VALID</div>";}
            } else {

                echo "<div style='border-radius:20px;color:black;margin-top:20px;margin-right:100px;;background-color:red;padding:20px;font-weight:bolder;font-size:larger;text-transform:uppercase'> EMAIL OR USERNAME ALREADY EXISTS</div>";
            }
        }
    }
    ...
    if (isset($_POST["emailS"]) && isset($_POST["passwordS"])) {
        if (!empty($_POST["emailS"]) && !empty($_POST["passwordS"])) {
            ...
            if ($line > 0) {
                ...
                echo "<p style='border-radius:20px;color:black;margin-right:100px;;margin-top:20px;background-color:red;padding:20px;font-weight:bolder;font-size:larger;text-transform:uppercase'> either you have a wrong PASSWORD OR  THIS EMAIL DOESN'T EXIST <p>";
            }
        }
    }
