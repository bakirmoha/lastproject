...
if (isset($_POST["emailS"]) && isset($_POST["passwordS"])) {
    if (!empty($_POST["emailS"]) && !empty($_POST["passwordS"])) {
        session_start();
        $_SESSION["emailS"] = htmlspecialchars($_POST["emailS"]);
...
if($DATA["TEL"]!==null && $DATA["ADRESSE"]!==null && $DATA["domain"]!==null )
{
    ...
    $_SESSION["TEL"]=$DATA["TEL"];$_SESSION["ADR"]=$DATA["ADRESSE"];$_SESSION["domain"]=$DATA["domain"];
...
else
{
    header("location:../acceuil1-main/condidat.php");
            ...
            echo "<p style='color:black;margin-top:20px;background-color:red;padding:20px;font-weight:bolder;font-size:larger;text-transform:uppercase'> either you have a wrong PASSWORD OR  THIS EMAIL DOESN'T EXIST <p>";
        }
    }
}
